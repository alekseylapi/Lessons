...
// Возвращаемое значение:
// 3
    public static function task7(array $array)
    {
        $sum = 0;
...
//переписать на цикл
//function task7(array $array) : ?float
//{
//    $sum = array_sum($array);
//    $count = count($array);
//    return $sum / $count;
...
//5 // число состоит из 5 цифр
    public static function task10(int $number): int
    {
        if ($number == 0) {
            return 1;
        }

        // Используем логарифм по основанию 10 для определения количества разрядов
...
//15 // 5 + 4 + 3 + 2 + 1 = 15
    ...
    public static function task12($number)
    {
        $sum = 0;
        $i = 0;
        $e = $number;
...
//12345
    ...
    public static function task13($number)
    {
        $revers = 0;
        $numDigits = task10($number);
        for ($i = 0; $i < $numDigits; $i++) {
            $digit = task11($number, $i);
